...
public class DeleteProductImageEndpoint : ApiEndpoint
{
    public override void MapEndpoint(IEndpointRouteBuilder app) =>
        app.MapDelete(
                "api/v1.0/admin/products/{productId:guid}/image/{imageId:guid}",
                async (
                    Guid productId,
                    Guid imageId,
                    ...
                    IMapper mapper,
                    ILogger<DeleteProductImageEndpoint> logger
                ) =>
                {
                    var command = mapper.Map<DeleteProductImageCommand>(
                        (currentUser.GetCurrentUserId(), productId, imageId)
                    );
                    ...
                    return await HandleAsync(
                        ...
                        success =>
                            Results.Ok(
                                SuccessResult.Success("Successfully deleted product image.")
                            ),
