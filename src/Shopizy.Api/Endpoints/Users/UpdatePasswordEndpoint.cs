...
public class UpdatePasswordEndpoint : ApiEndpoint
{
    ...
    {
        app.MapPatch(
                "api/v1.0/users/{userId:guid}/password",
                async (
                    Guid userId,
                    ...
                    IMapper mapper,
                    ILogger<UpdatePasswordEndpoint> logger
                ) =>
                {
                    if (user.AuthorizeOwner(userId, "this user's password") is { } forbidden)
                        return forbidden;
                    ...
                    return await HandleAsync(
                        ...
                        command,
                        success =>
                            Results.Ok(SuccessResult.Success("Successfully updated password.")),
