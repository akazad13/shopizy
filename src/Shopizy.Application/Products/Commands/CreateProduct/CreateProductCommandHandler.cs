...
public class CreateProductCommandHandler(IProductRepository _productRepository, ICloudinaryMediaUploader _mediaUploader) : IRequestHandler<CreateProductCommand, ErrorOr<Product>>
{
    public async Task<ErrorOr<Product>> Handle(CreateProductCommand request, CancellationToken cancellationToken)
    {
        ...
        var product = Product.Create(request.Name, request.Description, CategoryId.Create(request.CategoryId), request.Sku, request.StockQuantity, Price.CreateNew(request.UnitPrice, request.Currency), request.Discount, request.Brand, request.Barcode, request.Tags, "", productImages);
