...
public class CreateProductCommandHandler(IProductRepository _productRepository) : IRequestHandler<CreateProductCommand, ErrorOr<Product>>
{
    ...
    {
        var product = Product.Create(request.Name, request.Description,request.CategoryId, request.Sku, request.StockQuantity, Price.CreateNew(request.UnitPrice, request.Currency), request.Discount, request.Brand, request.Barcode, request.Tags, "");
