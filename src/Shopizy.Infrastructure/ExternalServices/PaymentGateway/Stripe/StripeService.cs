...
public class StripeService(TokenService tokenService,
        CustomerService customerService,
        ChargeService chargeService,
                SessionService sessionService) : IPaymentService
{
    ...
    public async Task<ErrorOr<CustomerResource>> CreateCustomer(string email, string name, string number, string expiryYear, string expiryMonth, string cvc, CancellationToken cancellationToken)
    {
        try
        {
    ...
    public async Task<ErrorOr<ChargeResource>> CreateCharge(string currency, long amount, string receiptEmail, string customerId, string description, CancellationToken cancellationToken)
    {
        try
        {
    ...
    )
    {
        try
        {
            var options = new SessionCreateOptions
            {
                ...
                LineItems =
                [
                    new SessionLineItemOptions { PriceData = new SessionLineItemPriceDataOptions {
