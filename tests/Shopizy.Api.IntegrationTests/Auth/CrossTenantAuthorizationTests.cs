...
public class CrossTenantAuthorizationTests(IntegrationTestWebAppFactory factory)
    : BaseIntegrationTest(factory)
{
    private async Task<Guid> SetupProductAsync()
    {
        ...
        var prodResponse = await HttpClient.PostAsJsonAsync(
            ...
            new CreateProductRequest(
                $"AuthZ Prod {Guid.NewGuid().ToString()[..4]}",
                "Short",
                "Full desc",
                category!.Id,
                10.00m,
    ...
    public async Task GetOrder_AsDifferentUser_ReturnsForbidden()
    {
        ...
        var response = await HttpClient.GetAsync(
            $"/api/v1.0/users/{userAId}/orders/{orderId}",
            TestContext.Current.CancellationToken
        );
    ...
    public async Task ProcessPayment_AsDifferentUser_ReturnsForbidden()
    {
        ...
        var response = await HttpClient.PostAsJsonAsync(
            $"/api/v1.0/users/{userAId}/payments",
            paymentRequest,
            TestContext.Current.CancellationToken
        );
