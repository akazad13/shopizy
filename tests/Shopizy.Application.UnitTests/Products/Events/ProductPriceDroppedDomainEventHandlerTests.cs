...
public class ProductPriceDroppedDomainEventHandlerTests
{
    private readonly Mock<IWishlistRepository> _mockWishlistRepository;
    private readonly Mock<IUserRepository> _mockUserRepository;
    private readonly Mock<IEmailService> _mockEmailService;
    private readonly Mock<ILogger<ProductPriceDroppedDomainEventHandler>> _mockLogger;
    private readonly ProductPriceDroppedDomainEventHandler _handler;

    public ProductPriceDroppedDomainEventHandlerTests()
    {
        _mockWishlistRepository = new Mock<IWishlistRepository>();
        _mockUserRepository = new Mock<IUserRepository>();
        _mockEmailService = new Mock<IEmailService>();
        _mockLogger = new Mock<ILogger<ProductPriceDroppedDomainEventHandler>>();

        _handler = new ProductPriceDroppedDomainEventHandler(
    ...
    public async Task Handle_WhenNoWishlistsContainProduct_ShouldNotSendEmails()
    {
        ...
        var domainEvent = new ProductPriceDroppedDomainEvent(product, 100m, 80m);
    ...
    [Fact]
    public async Task Handle_WhenWishlistsContainProduct_ShouldSendPriceDropEmail()
    {
        // Arrange
        var product = ProductFactory.CreateProduct();
        var domainEvent = new ProductPriceDroppedDomainEvent(product, 100m, 80m);
        ...
        _mockEmailService.Verify(
            e =>
                e.SendAsync(
                    user.Email,
                    It.Is<string>(s => s.Contains("Price Drop Alert")),
